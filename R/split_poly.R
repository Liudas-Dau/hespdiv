...
#' #Dividing a polygon
#' upper_half <- split_poly(polygon = poly, split_ids = c(1,12), min_id = 1, trivial_side = F, poli_side = T)
#' lower_half <- split_poly(polygon = poly, split_ids = c(1,12), min_id = 1, trivial_side = F, poli_side = F)
...
#' #Dividing a polygon
#' left_half <- split_poly(polygon = poly, split_ids = c(1,4), min_id = 1, trivial_side = F, poli_side = T)
#' right_half <- split_poly(polygon = poly, split_ids = c(1,4), min_id = 1, trivial_side = F, poli_side = F)
...
#' #Dividing a polygon
#' upper_half <- split_poly(polygon = poly, split_ids = c(1,6), min_id = 1, trivial_side = F, poli_side = T)
#' lower_half <- split_poly(polygon = poly, split_ids = c(1,6), min_id = 1, trivial_side = F, poli_side = F)
#' lines(upper_half,col=4)
#' lines(lower_half,col=2)
#' @export

split_poly<-function(polygon,split_ids,min_id=NULL, trivial_side=T, poli_side){
  if (is.null(min_id)){
        ...
        while (i < nrow(pol)){
          if (pol[i,1]>polygon[min_x_id,1] & pol[i,1] < polygon[max_x_id,1] & i>1){
            ...
            if ((pol[i,1]<=polygon[min_x_id,1] | pol[i+1,1]<=polygon[min_x_id,1]) &
              ...
              side <- c(side,pt_on_line(X=c(pol[i,1],pol[i+1,1]),Y=c(pol[i,2],pol[i+1,2]),x3=mid_x,ind = c(1,2))>zero)
          ...
          while (i < nrow(pol)){
            if (pol[i,2]>polygon[min_x_id,2] & pol[i,2] < polygon[max_x_id,2] & i>1){
              ...
              if ((pol[i,2]<=polygon[min_x_id,2] | pol[i+1,2]<=polygon[min_x_id,2]) &
                ...
                inds<-c(inds,i)
                side <- c(side,pt_on_line(X=c(pol[i,1],pol[i+1,1]),Y=c(pol[i,2],pol[i+1,2]),y3=mid_y,ind = c(1,2))<zero)
          ...
          while (i < nrow(pol)){
            if (pol[i,2]>y_b1_pts[i] & pol[i,2] < y_b2_pts[i] & i>1){
              ...
              if ((pol[i,2]<=y_b1_pts[i] | pol[i+1,2]<=y_b1_pts[i+1]) &
                ...
                side <- c(side,pt_on_line(X=c(pol[i,1],pol[i+1,1]),Y=c(pol[i,2],pol[i+1,2]),x3=mid_x,ind = c(1,2))>mid_y)
